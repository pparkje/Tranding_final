...
Members keyH = (Members) session.getAttribute("key");
session.setAttribute("key", keyH);
	...
	<div class="looking_center">
		<div class="looking_top">
			<ul class="looking_topmenu">
				<li><a href="javascript:void(0);">${output.g_detail_a}</a></li>&nbsp;
				<li><img src="${pageContext.request.contextPath}/views/tranding/img/right_right.png"></li>&nbsp;
				<li><a href="javascript:void(0);">${output.g_detail_b}</a></li>
			</ul>
		</div>
		<div class="look_main">
			<div class="look_img">
				<div class="look_img1">
					<a href="${pageContext.request.contextPath}/views/tranding/img/${output.g_image_a}" data-lightbox="myphoto-1">
						<img src="${pageContext.request.contextPath}/views/tranding/img/${output.g_image_a}" alt="인터로킹 가방 사진" />
					</a>
				</div>
			</div>
			<div class="look_order">
				...
				<div class="order_imfor2">
					<ul>
						<li>정가</li>
						<li>${output.g_ori_price}</li>
						<li>원</li>
						<li>트렌딩판매가</li>
						<li>${output.g_price}원&nbsp;</li>
						<li>(관부가세 포함)&nbsp;</li>
						<li>${output.g_discount}</li>
						<li>결제혜택가</li>
						<li>${output.g_disc_price}&nbsp;</li>
						<li class="brand_hover">!</li>
						<li>D머니</li>
						<li>최대 ${output.g_point}D&nbsp;</li>
				...
				<div class="order_imfor3">
					<ul>
						...
						<%
							if(keyH == null) {
						%>
						<c:url value="/pjh/jsp/looking_cartpage_login.do" var="loginA">
							<c:param name="id" value="${output.id}" />
						</c:url>
						<li class="baro_gume_A"><a href="${loginA}" class="baro_gume">바로 구매하기</a></li>
						<% 
						}else{
	                    	session.setAttribute("key", keyH);
						%>
						<c:url value="/pjh/jsp/cart_order.do" var="loginB" >
							<c:param name="id" value="${output.id}" />
						</c:url>
						<li class="baro_gume_A"><a href="${loginB}" class="baro_gume" >바로 구매하기</a></li>
						<% } %>
						<%
							if(keyH == null) {
						%>
						...
						<% 
	                    	...
	                    	session.setAttribute("key", keyH);
						%>
						<c:set var="m_id" value="${outputA.m_id}" />
			...
			<div class="look_information">
				<div class="guidance1">
					<ul>
						<li>&nbsp;&nbsp;
						<c:url value="/pjh/jsp/event_contents.do" var="event_A_A">
							<c:param name="id" value="1" />
						</c:url>
						<a href="${event_A_A}">
						[안내] ${outputJ[0].n_subject}
						</a>
						</li>
						<li>&nbsp;&nbsp;
						<c:url value="/pjh/jsp/event_contents.do" var="event_A_B">
							<c:param name="id" value="2" />
						</c:url>
						<a href="${event_A_B}">
						[안내] ${outputJ[1].n_subject}
						</a>
						</li>
					</ul>
				</div>
		...
		<div class="swiper-container">
			...
			<c:forEach var="i" begin="1" end="23" step="2" varStatus="status">
				<!-- 슬라이드 처리 영역  -->
				<% 
                    	if(keyH == null){ 
	            %>
				...
				<div class="swiper-slide">
					<div class="sub_imformation">
						...
						<a href="${viewUrlA}">
							<img src="${pageContext.request.contextPath}/views/tranding/img/${outputB[status.index-1].g_image_a}" class="looking_img">
							<ul class="sub_imfor1">
								<li>${outputB[status.index-1].g_brand}</li>
								<li>${outputB[status.index-1].g_name}</li>
								<li>${outputB[status.index-1].g_count_memprice_price}</li>
								<li>${outputB[status.index-1].g_discount}</li>
								<li>${outputB[status.index-1].g_ori_price}</li>
							</ul>
						</a>
					</div>
				</div>
				<% }else{ 
                    	session.setAttribute("key", keyH);
	            %>
	            <c:url value="/pjh/jsp/looking.do" var="viewUrlB">
					<c:param name="id" value="${status.index}" />
				</c:url>
				<div class="swiper-slide">
					<div class="sub_imformation">
		...
		<div class="looking_looking_looking">
			...
			<div class="looking_imformation_img_look">
				<div class="looking_imformation_img_look1">
					...
					<c:choose>
						<c:when test="${output.g_image_b == null || fn:length(output.g_image_b) == 0}">
							<h1>조회결과가 없습니다.</h1>
						</c:when>
						<c:otherwise>
							<c:set var="photo" value="${fn:split(output.g_image_b,',')}" />
	...
	<div class="review_looking">리뷰</div>
		<div class="swiper-container">
			...
			<c:forEach var="i" begin="1" end="12" step="1" varStatus="status">
				<!-- 슬라이드 처리 영역  -->
	            <c:url value="/pjh/jsp/review_info.do" var="review">
					<c:param name="outputA" value="${status.index}" />
				</c:url>
				<div class="swiper-slide">
					<div class="sub_imformation">
						<a href="${review}">
							<img src="${pageContext.request.contextPath}/views/tranding/img/${outputC[status.index-1].r_img}" class="looking_img">
							<ul class="sub_imfor1">
								<li>
								${outputC[status.index-1].r_cont}
								</li>
							</ul>
						</a>
					</div>
		...
		<div class="review_write1">
			<%
				if(keyH == null) {
			%>
			<a href="${pageContext.request.contextPath}/pjh/jsp/looking_cartpage_login.do">리뷰작성</a>
			<% 
				}else{
	            session.setAttribute("key", keyH);
			%>
			<a href="${pageContext.request.contextPath}/pjh/jsp/review_plus.do">리뷰작성</a>
			<%
				}
			%>
		</div>
