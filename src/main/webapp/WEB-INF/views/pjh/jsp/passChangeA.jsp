...
Members keyM = (Members) session.getAttribute("key");
session.setAttribute("key", keyM);

%>
    <div class="main wrap"> 
		<div class="rightDiv">
			<h2>비밀번호 변경 ③</h2>
			<form action="${pageContext.request.contextPath}/pjh/jsp/passChange_ok_A.do">
				<input type="hidden" value="${output.id}" name="id"/>
				<input type="hidden" value="${output.m_id}" name="m_id"/>
				신규 비밀번호 <input type="password" name="new_m_pw_A" class="sin_password_A">
				<br/>
				신규 비밀번호 확인<input type="password" name="new_m_pw_B" class="sin_password_B">
				<br/>
				<span>
