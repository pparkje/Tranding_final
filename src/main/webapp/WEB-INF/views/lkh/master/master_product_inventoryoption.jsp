...
<head>
    ...
    <style type="text/css">
    	...
    	<%@ include file="../css/master_product_inventoryoption.css" %>
    </style>
</head>

<body>
    <%@ include file="../jsp/master_header_inventoryoption.jsp" %>
    <%@ include file="../jsp/master_product_sidebar.jsp" %>
    <!-- ================= 본문영역 시작 ================= -->
                    <%
	request.setCharacterEncoding("UTF-8");
	Admins keyB_J = (Admins) session.getAttribute("keyA");
    session.setAttribute("keyA", keyB_J);
%>
    <div class="Main_place">
        <!-- 이 안에서 내용 작업하시면 됩니다. -->
        
        <div class="main_place1 clearfix">
            <a href="${pageContext.request.contextPath}/tranding/master_product_inventoryoption.do"><button type="button" class="button_property3 pull-left">전체목록</button></a>
        ...
        <div class="main_place2">
            <form name="product_form" method="get" action="${pageContext.request.contextPath }/tranding/master_product_inventoryoption.do">
        ...
        <div class="main_place3">
            <table border="1" width="80%">
                <thead>
                    <tr>
                        ...
                        <th height="30px">상품명</th>
                        <th width="170px">옵션항목</th>
                        <th width="150px">옵션타입</th>
                        <th width="90px">창고재고</th>
                        <th width="90px">주문대기</th>
                        <th width="70px">판매</th>
                        <th width="70px">관리</th>
                    </tr>
                </thead>
		    			...
		    			<c:otherwise>
		    				...
		    				<c:forEach var="item" items="${output }" varStatus="status">
		    					<tr>
			                        ...
			                        <td width="70px" align="center">
			                            <a href="${pageContext.request.contextPath}/tranding/master_product_update.do?id=${item.id }&g_detail_a=${item.g_detail_a }&g_detail_b=${item.g_detail_b }&way=master_product_inventoryoption"><button>수정</button></a>
			                        </td>
			                    </tr>
		    				</c:forEach>
		    			</c:otherwise>
		    ...
		    <c:choose>
		    	...
		    	<c:when test="${pageData.prevPage > 0 }">
		    		<%-- 이동할 URL 생성 --%>
		    		<c:url value="/tranding/master_product_inventoryoption.do" var="prevPageUrl">
		    ...
		    <c:forEach var="i" begin="${pageData.startPage }" end="${pageData.endPage }" varStatus="status">
		    	<%-- 이동할 URL 생성 --%>
		    	<c:url value="/tranding/master_product_inventoryoption.do" var="pageUrl">
		    ...
		    <c:choose>
		    	...
		    	<c:when test="${pageData.nextPage > 0 }">
		    		<%-- 이동할 URL 생성 --%>
		    		<c:url value="/tranding/master_product_inventoryoption.do" var="nextPageUrl">
