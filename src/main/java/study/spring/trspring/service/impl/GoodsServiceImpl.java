...
public class GoodsServiceImpl implements GoodsService{
	...
	public List<Goods> getGcount(Goods input) throws Exception {
		...
		try {
			result = sqlSession.selectList("GoodsMapper.selectListGcount", input);
...
//	브랜드 관련조회
	...
	public List<Goods> getGoodsSearch(Goods input) throws Exception {
		...
		try {
			result = sqlSession.selectList("GoodsMapper.selectList_search", input);
	...
	public int getGoodsCountSearch(Goods input) throws Exception {
		...
		try {
			result = sqlSession.selectOne("GoodsMapper.selectList_search_count", input);
		} catch(Exception e) {
			log.error(e.getLocalizedMessage());
			throw new Exception("데이터 조회에 실패했습니다.");
		}
